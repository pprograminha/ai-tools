...
type VideosContextProps = {
  ...
  finishedConversionAt: Date | undefined;
  finishedTranscriptionAt: Date | undefined;
  startAudioConversion: (videos: Map<string, Video>) => Promise<void>;
}
...
export function VideosProvider({children}: {children: ReactNode}) {
  ...
  async function convertVideoToAudio(id: string, video: Video) {
    dispatch({
  ...
  async function startAudioConversion(videos: Map<string, Video>) {
    dispatch({ type: ActionTypes.START_CONVERSION })


    for (const [id, video] of videos.entries()) {
      await convertVideoToAudio(id, video)
    }
